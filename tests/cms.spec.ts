...
test.describe('CMS Content Validation - DotCMS Demo', () => {
  ...
  test('homepage should load with dynamic content sections', async () => {
    ...
    // Demo site resets twice daily, so check that at least some content exists
    const totalContent = activityCount + productCount + eventCount + blogCount;
    expect(totalContent).toBeGreaterThan(0);

    console.log(
      `Content sections - Activities: ${activityCount}, Products: ${productCount}, Events: ${eventCount}, Blog: ${blogCount}`,
    );
  });
  ...
  test('blog posts should have links from CMS', async ({ page }) => {
    ...
    if (count === 0) {
      console.log('No blog posts on homepage (demo site may have reset)');
  ...
  test('navigating to blog section should load CMS content', async ({ page }) => {
    ...
    if (count === 0) {
      const pageUrl = page.url();
      expect(pageUrl).toContain('/blog');
      console.log('Blog page loaded but no specific entries found (demo site state)');
    } else {
      expect(count).toBeGreaterThan(0);
    }
  });
